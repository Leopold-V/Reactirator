...
export const TreeItem = ({
  ...
  const editNameItem = () => {
    console.log('todo edit name item');
  }
    ...
    <li>
      <div className={`flex items-center justify-between hover:bg-gray-100 cursor-pointer`} onMouseEnter={displayButtons} onMouseLeave={hideButtons}>
        ...
        <div className={`${visible ? 'visible' : 'hidden'} flex items-center space-x-1`}>
            ...
            <button onClick={removeItem}>
                <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-trash transform hover:rotate-45 transition duration-200" width="20" height="20" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#3d3d3d" fill="none" strokeLinecap="round" strokeLinejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      ...
      <ul className="ml-4">
        {childrenItems.map((ele) => (
          <TreeItem key={ele.name} structure={structure} name={ele.name} isFolder={ele.isFolder} dispatchStructure={dispatchStructure} />
        ))}
      </ul>
    </li>
  );
