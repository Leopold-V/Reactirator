...
export const TreeItem = ({ structure, name, isFolder } : { structure: structureStateType, name: string, isFolder: boolean }) => {
    const childrenItems = structure.filter((ele) => ele.ancestor === name);

    return (
        <li>
            <div className={`${isFolder ? "space-x-1" : ""} flex items-center `}>
                {isFolder ? <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-folder" width="20" height="20" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#7c5f01" fill="#ffe368" strokeLinecap="round" strokeLinejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                    <path d="M5 4h4l3 3h7a2 2 0 0 1 2 2v8a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-11a2 2 0 0 1 2 -2" />
                </svg> :
                <svg version="1.1" id="Layer_2_1_" xmlns="http://www.w3.org/2000/svg" width="27" height="27" viewBox="0 0 841.9 595.3">
                <g>
            ...
            <ul className="ml-4">
                {childrenItems.map((ele) => <TreeItem key={ele.name} structure={structure} name={ele.name} isFolder={ele.isFolder} />)}
            </ul>
        </li>
    )
}
