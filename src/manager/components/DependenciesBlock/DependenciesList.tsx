...
export const DependenciesList = () => {
  ...
  return (
    ...
    {Object.entries(dependencies.dependencies).map((ele) => (
      <DependenciesItem key={ele[0]} depName={ele[0]} depVersion={ele[1].version} isDevDep={false} status={ele[1].status} />
    ))}
    {Object.entries(dependencies.devDependencies).map((ele) => (
      <DependenciesItem key={ele[0]} depName={ele[0]} depVersion={ele[1].version} isDevDep={true} status={ele[1].status} />
    ))}
  </ul>
  )
};